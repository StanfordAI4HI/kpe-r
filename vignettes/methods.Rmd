...
right-hand term is the expected outcome under the best single
treatment applied to every individual.
...
Concatenating the per-block $\psi$ vectors across all $G$ blocks yields
a length-$n$ vector in which each sample is evaluated exactly once; the
shuffle-level summary is $\hat\psi_s = \text{mean}$ and
$\hat\sigma_s = \text{std}$.

Unlike the original Algorithm 2 (Li and Brunskill, Science 2026) 
--- which fits the policy and reward
models on *disjoint* subsets of the non-evaluation folds --- 
kpe2 fits
...
second term accounts for across-shuffle variability introduced by
sample splitting.

The one-sided upper-tail t-statistic is
$t = \hat\psi / \sqrt{\widehat{\operatorname{Var}}(\hat\psi)}$, and
the reported `p_value` is the corresponding upper-tail probability
under $t_{n-1}$. The `confidence_interval` is the 95% Wald interval
$\hat\psi \pm 1.96 \sqrt{\widehat{\operatorname{Var}}(\hat\psi)}$.


## Stability diagnostic

`policy_stability` is the fraction of samples for which every
shuffle's contextual policy predicted the same action; its range is
$[0, 1]$. `overall_stability` is the analogous quantity for the
best-arm policy.
